...
class StackTrainer(BaseModel):
    ...
    def forward_generator(self):
        self.style_feat = self.language_model(self.text_input)
        self.miss_style_feat = self.language_model(self.miss_input)
        ...
        images = self.generator(self.style_feat.to(self.device_map['img']).detach(), torch.randn_like(self.style_feat).to(self.device_map['img']))
        inter_s = self.generator(self.interp_style.to(self.device_map['img']).detach(), torch.randn_like(self.style_feat).to(self.device_map['img']))
    ...
    def backward_generator(self):
        ...
        loss += self.loss_consistency * 0.5
        loss.backward()
