...
class LanguageModel(nn.Module):
    def __init__(self, cfg, tokenizer, device):
        super().__init__()
        self._config = transformers.configuration_bert.BertConfig(
        ...
        self._model_base = transformers.modeling_bert.BertModel(self._config)
    ...
    def forward(self, input):
        if not isinstance(input, list):
            input = [input]
        ids = [self.tokenizer.encode(string).ids for string in input]
        len_ids = [len(sentence) for sentence in ids]
        pad_ids = [sentence[:128] + [self.pad_token] * (128 - length) for (sentence, length) in zip(ids, len_ids)]
        pad_ids = torch.tensor(pad_ids).to(self.device)
        sequence_output, pooled_output = self._model_base(input_ids=pad_ids)
...
class StackTrainer(BaseModel):
    def __init__(self, cfg):
        ...
        self.language_model = LanguageModel(cfg, tokenizer, self.device_map['style']).to(self.device_map['style'])
