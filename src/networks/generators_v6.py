...
class RNN(nn.Module):
    def __init__(self, cfg, vocab_size):
        ...
        self._net = nn.LSTM(cfg.embedding_size, self.hidden_dim, self.hidden_layers, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):
        ...
        hid = hid.view(emb.size(0), self.hidden_layers, 2, hid.size(1), hid.size(2))
        hid = hid[:, -1, :, :, :]
...
class StyleNetwork(nn.Module):
    def __init__(self, cfg):
        ...
        _style_net = [LinearBlock(self.input_dim, self._proj_dim, act='leaky_relu', norm='layer', spectral=cfg.gen_spectral)]
        for _ in range(cfg.style_layers):
            _style_net += [LinearBlock(self._proj_dim, self._proj_dim, act='leaky_relu', norm='layer', spectral=cfg.gen_spectral)]
        self._style_net = nn.Sequential(*_style_net)

        _mapping_net = []
        for scale in style_scales:
            _mapping_net += [LinearBlock(self._proj_dim, scale, act='leaky_relu', norm='none', spectral=cfg.gen_spectral)]
...
class StyleBlock(nn.Module):
    def __init__(self, ic, oc, ks=3, pad='reflect', act='leaky_relu', upsample=False, spectral=False):
        global style_scales
        super().__init__()
        self.k = min(ic, oc)
...
class StyleGenerator(nn.Module):
    def __init__(self, cfg):
        ...
        generator_bodies = []
        generator_bodies += [GeneratorTrunk(cfg.ngf * 8, cfg.ngf * 4, num_block=2, spectral=cfg.gen_spectral)]
        generator_bodies += [GeneratorTrunk(cfg.ngf * 4, cfg.ngf * 2, num_block=2, spectral=cfg.gen_spectral)]
        generator_bodies += [GeneratorTrunk(cfg.ngf * 2, cfg.ngf * 1, num_block=2, spectral=cfg.gen_spectral)]
    ...
    def forward(self, input_style, input_content):
        ...
        channels_z = 16 * self.cfg.ngf - self.latent_channels

        init_z = torch.randn(size=(content.size(0), channels_z, self.latent_scale, self.latent_scale)).to(content.device)
        feats = torch.cat([content, init_z], dim=1)
        ...
        for _layer, _head in zip(self.generator_bodies, self.generator_heads):
            feats = _layer(feats, style_pairs[idx:idx+2])
            results += [_head(feats)]
            idx = idx + 2
