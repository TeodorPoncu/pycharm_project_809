...
class StyleGenerator(nn.Module):
    def __init__(self, cfg):
        ...
        generator_bodies = []
        generator_bodies += [StyleBlock(cfg, cfg.ngf * 8, cfg.ngf * 4, upsample=True)]
        generator_bodies += [StyleBlock(cfg, cfg.ngf * 4, cfg.ngf * 2, upsample=True)]
        generator_bodies += [StyleBlock(cfg, cfg.ngf * 2, cfg.ngf * 1, upsample=True)]
    ...
    def forward(self, input_style, input_content):
        ...
        for _layer, _head in zip(self.generator_bodies, self.generator_heads):
            ...
            if len(results):
                results += [(self.upsample(results[-1]) + _head(feats)) * 0.5] # avoid overflow
            else:
                results += [_head(feats)]
        return results
